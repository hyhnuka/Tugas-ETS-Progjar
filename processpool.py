...
def main():
    ...
    srv.listen()
    logging.info(f"[ProcessPool] Listening on port {PORT} with {WORKER_LIMIT} workers")

    with ProcessPoolExecutor(max_workers=WORKER_LIMIT) as pool:
        while True:
            conn, _ = srv.accept()
            fd = os.dup(conn.fileno())
            pool.submit(handle_conn_fd, fd)
            conn.close()
